...
## Acceptance Criteria
- [ ] Database column hidden when all data comes from a single database
- [ ] Database filter dropdown hidden when all data comes from a single database
- [ ] Both appear when multiple databases are present
- [ ] Applies to both CallView and CollectionView

## Done Condition
Database column and filter are only visible when there are multiple databases.
